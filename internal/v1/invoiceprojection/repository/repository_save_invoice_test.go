...
func TestSaveInvoiceSuccess(t *testing.T) {
	...
	invoiceMock := NewInvoiceBuilder().
		...
		AddValue(500.50).
		AddUserId("User1").
		AddInvoiceProjectionId("7a494375-53a1-41e4-a9db-6bb30eaf23c2").
		Build()
	...
	invoiceSaved, err := _repository.SaveInvoice(context.Background(), *invoiceMock)
...
func TestSaveInvoiceBeginFail(t *testing.T) {
	...
	_, err = _repository.SaveInvoice(context.Background(), *invoiceMock)
	assert.Error(t, err)
